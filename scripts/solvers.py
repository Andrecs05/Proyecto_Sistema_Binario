...
def binary_potential_laplace(r, theta, M1, M2, a, G, n_terms):
    ...
    M = M1 + M2 # Total mass
    phi = -G * M / r # Monopole term
    a1 = M2 * a / M  # M1 position (right of COM)
    a2 = M1 * a / M  # M2 position (left of COM)

    for n in range(1, n_terms + 1):
        Pn = legendre(n) # Legendre polynomial of degree n
        Bn = -G * ((M1 * a1**n) + (M2 * (-a2)**n)) # Multipole coefficient
        phi += Bn / r**(n + 1) * Pn(np.cos(theta)) # Add multipole term
...
def binary_potential_poisson_fd(M1, M2, R_star1, R_star2, a, G, L, grid_size):
    ...
    rho = np.zeros((N, N)) # mass density array
    ...
    for i in range(N):
        for j in range(N):
            ...
            if r1 <= R_star1:
                rho[i, j] += M1 / area_1 # uniform density inside star 1
            if r2 <= R_star2:
                rho[i, j] += M2 / area_2 # uniform density inside star 2

    phi = np.zeros((N, N)) # potential array

    N_total = N * N

    # Construct the sparse matrix A and right-hand side vector b
    main_diag = -4 * np.ones(N_total)
    off_diag1_r = np.ones(N_total - 1)
    off_diag1_l = np.ones(N_total - 1)
    off_diagN = np.ones(N_total - N)

    # Adjust for wrap-around at boundaries
    for i in range(N):
        for j in range(N - 1):
            k = i * N + j
            if j == N - 1:
    ...
    switching_radius = a + max(R_star1, R_star2)  # radius beyond which to apply Laplace solution
    ...
    phi = spsolve(A, b) # solve the linear system
    phi = phi.reshape((N, N))
